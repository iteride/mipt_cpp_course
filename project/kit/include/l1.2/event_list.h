...
// событие: список — окно последних, а не архив. Значит, после вызова всегда
// size <= capacity, и это то, что проверяют тесты.
void ListPushBack(EventList* list, const Event* event);

// Убирает самое старое событие. На пустом списке — ничего не делает
// и не считается ошибкой: «убрать из пустого» — обычный ход событий,
